...
**Convenience PA** beeps in the earpiece as soon as the call answers (Protect talkback + volume arm in the background; mic audio buffers). Campus preamble is off by default — set `PA_PREAMBLE_FILE=Test_Start_Tone.mp3` for a building tone before the mic opens.
